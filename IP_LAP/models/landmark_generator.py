...
class Fusion_transformer_encoder(nn.Module):
    ...
    def forward(self,ref_embedding,mel_embedding,pose_embedding):#(B,Nl,512)  (B,T,512)    (B,T,512)
        ...
        #(2)  modality encoding
        modality_v = self.modality(1 * torch.ones((pose_embedding.size(0), self.T), dtype=torch.int).to(self.device))
        modality_a = self.modality(2 * torch.ones((mel_embedding.size(0),  self.T), dtype=torch.int).to(self.device))
        ...
        ref_tokens = ref_embedding + self.modality(
            3 * torch.ones((ref_embedding.size(0), ref_embedding.size(1)), dtype=torch.int).to(self.device))
...
class Landmark_generator(nn.Module):
    def __init__(self,T,d_model,nlayers,nhead,dim_feedforward,dropout=0.1, device='cuda'):
        ...
        self.fusion_transformer = Fusion_transformer_encoder(T,d_model,nlayers,nhead,dim_feedforward,dropout,device)
